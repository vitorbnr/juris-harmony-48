...
import { User, Building2, Users, Plus, Settings, Shield, Activity, Check, Save } from "lucide-react";
...
function TabEquipe() {
  ...
  const carregar = useCallback(() => {
    setLoading(true);
    usuariosApi.listar().then((res: { content?: unknown[] } | unknown[]) => {
      const items = (res as { content?: unknown[] }).content ?? res;
...
// Preferências removido a pedido do usuário

function TabLogs() {
  const [logs, setLogs] = useState<Record<string, unknown>[]>([]);

  useEffect(() => {
    logsApi.listar().then((res: { content?: Record<string, unknown>[] } | Record<string, unknown>[]) => {
      const items = (res as { content?: Record<string, unknown>[] }).content ?? res;
    ...
    <div className="space-y-4">
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        {logs.length === 0 ? (
          ...
          <div key={log.id as string} className={cn("flex items-start gap-4 px-5 py-4 hover:bg-muted/30 transition-colors", i < logs.length - 1 && "border-b border-border/50")}>
            <Avatar className="h-8 w-8 border border-primary/20 shrink-0 mt-0.5">
              <AvatarFallback className="bg-primary/10 text-primary text-xs font-semibold">
                {String(log.usuarioNome || "SYS").split(" ").slice(-2).map((n: string) => n[0]).join("")}
              </AvatarFallback>
            </Avatar>
